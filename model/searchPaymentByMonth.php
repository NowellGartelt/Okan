...
class searchPaymentByTransaction {
 ...
 public function searchPaymentByTransaction($payName, $payCategory, $payState, $payDateFrom, $payDateTo) {
  ...
  // x = 10
   ...
   $query_referencePay = "SELECT * FROM paymentTable WHERE payName LIKE '%{$payName}%' 
    AND payCategory LIKE '%{$payCategory}%' AND payState LIKE '%{$payState}%' ORDER BY payDate ASC";
  } elseif ($payName !== "" && $payCategory !== "" && $payDateFrom !== "") {
   $query_referencePay = "SELECT * FROM paymentTable WHERE payName LIKE '%{$payName}%' 
    AND payCategory LIKE '%{$payCategory}%' AND payDate <= '$payDateFrom' ORDER BY payDate ASC";
  } elseif ($payName !== "" && $payState !== "" && $payDateFrom !== "") {
   $query_referencePay = "SELECT * FROM paymentTable WHERE payName LIKE '%{$payName}%' 
    AND payState LIKE '%{$payState}%' AND payDate <= '$payDateFrom' ORDER BY payDate ASC";
  } elseif ($payCategory !== "" && $payState !== "" && $payDateFrom !== "") {
   $query_referencePay = "SELECT * FROM paymentTable WHERE payCategory LIKE '%{$payCategory}%' 
    AND payState LIKE '%{$payState}%' AND payDate <= '$payDateFrom' ORDER BY payDate ASC";
  } elseif ($payName !== "" && $payCategory !== "" && $payDateTo !== "") {
   $query_referencePay = "SELECT * FROM paymentTable WHERE payName LIKE '%{$payName}%' 
    AND payCategory LIKE '%{$payCategory}%' AND payDate <= '$payDateTo' ORDER BY payDate ASC";
  } elseif ($payName !== "" && $payState !== "" && $payDateTo !== "") {
   $query_referencePay = "SELECT * FROM paymentTable WHERE payName LIKE '%{$payName}%' 
    AND payState LIKE '%{$payState}%' AND payDate <= '$payDateTo' ORDER BY payDate ASC";
  ...
  // x = 5
   ...
   $query_referencePay = "SELECT * FROM paymentTable 
   　WHERE payName LIKE '%{$payName}%' ORDER BY payDate ASC";
  } elseif ($payCategory !== "") {
   $query_referencePay = "SELECT * FROM paymentTable 
   　WHERE payCategory LIKE '%{$payCategory}%' ORDER BY payDate ASC";
  } elseif ($payState !== "") {
   $query_referencePay = "SELECT * FROM paymentTable 
   　WHERE payState LIKE '%{$payState}%' ORDER BY payDate ASC";
  } elseif ($payDateFrom !== "") {
   $query_referencePay = "SELECT * FROM paymentTable 
   　WHERE payDate >= '$payDateFrom' ORDER BY payDate ASC";
  } elseif ($payDateTo !== "") {
   $query_referencePay = "SELECT * FROM paymentTable 
   　WHERE payDate <= '$payDateTo' ORDER BY payDate ASC";
  
  // ひとつも入力されていない場合
  // 全件検索する
  } else {
   $query_referencePay = "SELECT * FROM paymentTable ORDER BY payDate ASC";
